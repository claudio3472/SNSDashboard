...
def build_main_figure(dff):
    ...
    ts["rolling_gastos"] = (
        ts["gastos_operacionais"]
        .rolling(3)
        .mean()
    )

    ts["rolling_rendimentos"] = (
        ts["rendimentos_operacionais"]
        .rolling(3)
        .mean()
    )
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    className="card",
                    children=[
                        dcc.Graph(id="finance-orcamento")
                    ],
                ),
...
@callback(
    Output("finance-institution-filter", "options"),
    Output("finance-institution-filter", "value"),
    Input("finance-region-filter", "value"),
)
...
@callback(
    Output("store-range-finance", "data"),
    Input("finance-graph", "relayoutData"),
)
...
@callback(
    ...
    Output("finance-regiao", "figure"),
    Output("finance-orcamento", "figure"),
    Output("finance-divida", "figure"),
    Input("store-range-finance", "data"),
    Input("finance-region-filter", "value"),
    Input("finance-institution-filter", "value"),
)
def update_dashboard(
    ...
    if selected_institution != "all":
        dff_base = dff_base[
            dff_base["instituicao"] == selected_institution
        ]
    ...
    total_gastos = (
        dff["gastos_operacionais"].sum()
        / 1e6
    )

    total_rendimentos = (
        dff["rendimentos_operacionais"].sum()
        / 1e6
    )
